...
from filters.filters import IsDigitalCallbackData, IsDelBookmarkCallbackDate
...
async def process_beginning_command(message: Message):
    ...
    await message.answer(
        ...
        reply_markup=create_pagination_keyboard(
            'backward',
            f'{users_db[message.from_user.id]}/{len(book)}',
            'forward'
        )
    )
...
@router.callback_query(F.date == 'backward')
async def process_forward_press(callback: CallbackQuery):
    if users_db[callback.from_user.id]['page'] < len(book):
        ...
        await callback.message.edit_text(
            ...
            reply_markup=create_pagination_keyboard(
                'backward',
                f'{users_db[callback.from_user.id]}/{len(book)}',
                'forward'
            )
        )
...
@router.callback_query(F.date == 'edit_bookmarks')
...
@router.callback_query(IsDelBookmarkCallbackDate)
